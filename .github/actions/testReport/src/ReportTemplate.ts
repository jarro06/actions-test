...
export class ReportTemplate {
    ...
    private testSuitesData: TestSuiteData[];

    constructor(reportData: ReportData, testSuitesData: TestSuiteData[]) {
    ...
    getTestStatistics(): string {
        if (this.testSuitesData.length === 0) {
            return ""
        }

        let statisticPart: string = this.tableHeader();

        this.testSuitesData.forEach((testSuite: TestSuiteData) => {
            statisticPart += ReportTemplate.suiteStatistics(testSuite);
        });

        this.reportData.listWithFailedTestProjects.forEach((failedProject: string) => {
    ...
    getTestCaseDetails(): string {
        ...
        this.testSuitesData.forEach((testSuite: TestSuiteData) => {
            ...
            testSuite.testCasesData.forEach((testCase: TestCaseData) => {
    ...
    private static suiteStatistics(testSuite: TestSuiteData): string {
    ...
    private static failedProject(failedProject: string) {
        return `
        <tr>
    ...
    private static testsCaseHeader(testSuite: TestSuiteData): string {
        return `
        <table border=0 cellspacing=0 cellpadding=0 style='border-collapse:collapse;border:none; width:100%'>
        <tr>
    ...
    private static testCaseDetails(testCase: TestCaseData): string {
